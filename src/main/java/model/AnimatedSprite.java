...
public class AnimatedSprite extends ImageView {
    ...
    int count, columns, offsetX, offsetY, width, height, curXIndex=0, curYIndex=0, run = 0;

    public AnimatedSprite(Image image, int count, int columns, int offsetX, int offsetY, int width, int height){
        this.setNewImage(image, count, columns, offsetX, offsetY, width, height);
    }

    public void setNewImage(Image image, int count, int columns, int offsetX, int offsetY, int width, int height) {
        this.setImage(image);
        this.count = count;
